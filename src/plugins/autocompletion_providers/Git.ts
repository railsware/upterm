...
const commitOptionsData: OptionData[] = [
    ...
    {
        longFlag: "patch",
        description: "Use the interactive patch selection interface to chose which changes to commit. See git-add(1) for details.",
    },
    {
        longFlag: "null",
        shortFlag: "z",
        description: "When showing short or porcelain status output, terminate entries in the status output with NUL, instead of LF. If no format is given, implies the --porcelain output format.",
    },
    {
        longFlag: "template",
    ...
    },
    {
        ...
        description: "When used with -C/-c/--amend options, or when committing after a a conflicting cherry-pick, declare that the authorship of the resulting commit now belongs to the committer. This also renews the author timestamp.",
        shortFlag: null,
    },
    {
        longFlag: "short",
...
const commitOptions = combine(commitOptionsData.map(({ longFlag, shortFlag, description }) => {
    const provider = shortFlag === null ? Common.longFlag(longFlag) : longAndShortFlag(longFlag, shortFlag);
